...
- [ ] 1.1 **RED → GREEN → REFACTOR:** add `src/app.setup.spec.ts`/`test/app.e2e-spec.ts` for `/api`, whitelist/UUID 400-without-mutation, numeric output and Swagger; extract `src/app.setup.ts`, update `src/main.ts`, Swagger decorators and `package*.json`. Verify focused Jest, E2E, build.
- [ ] 1.2 **RED → GREEN → REFACTOR:** add DTO/mapper tests for canonical `unitPrice`, `subtotal`, `total`, `SHIPPED`; create `src/orders/mappers/*` and response DTOs, update order entities/controllers. Verify `npm test -- --runInBand orders`.

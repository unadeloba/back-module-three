...
- Run e2e tests with `docker compose up -d db && npm run test:e2e && docker compose stop db`; they hit the real bootstrap under `/api` against a real PostgreSQL instance (not a mocked `AppModule`).
...
- Order status follows a strict transition table (`PENDING → CONFIRMED → SHIPPED → DELIVERED`, or `CANCELLED` from `PENDING`/`CONFIRMED`) enforced in `orders.service.ts`; skips, reversals, and transitions from a terminal state are rejected. `PATCH /orders/:id/status` is the only route that mutates status.
- Cancellation restocking runs inside the same locked transaction as the status update: it sorts and locks every affected product with `pessimistic_write` before incrementing stock, so concurrent/duplicate cancellation requests restore each line's quantity exactly once — never lose the transaction boundary when touching this path.
...
- The Swagger contract at `GET /api/docs-json` is treated as ground truth for consumers (including the sibling `front-module-three` frontend) — any DTO/route change should be reflected in `@ApiProperty`/`@ApiResponse` decorators, not just runtime behavior. `test/app.e2e-spec.ts` has a field-by-field (non-snapshot) proof of this contract; keep it in sync.

## Verification

- For code changes, format and lint touched TypeScript first, then run `npm run build` and `npm test -- --runInBand`.
- Add the e2e command above when changing module wiring, HTTP routes, validation, entities, database behavior, or Swagger decorators.
- Full pre-merge evidence trail (task-by-task TDD evidence, verification reports, and archived specs) lives under `openspec/changes/archive/` — check there for the reasoning behind an existing invariant before changing it.
